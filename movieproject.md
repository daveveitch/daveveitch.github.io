...
<div class="output_html rendered_html output_subarea output_execute_result"><script>function code_toggle() { if (code_shown){ $('div.input').hide('500'); $('#toggleButton').val('Show Code') } else { $('div.input').show('500'); $('#toggleButton').val('Hide Code') } code_shown = !code_shown } $( document ).ready(function(){ code_shown=false; $('div.input').hide() });</script>
...
<center>RATING THE CRITICS</center>

## 

<center>An In-Depth Look at New York Times Film Critics and the Academy of Motion Picture Arts and Sciences Voting Membership</center>
...
Results from the API calls were written to CSV files which serve as inputs to this Jupyter Notebook. This was done in order to reduce the number of times the APIs had to be accessed. It was also done because some API calls had to be made over multiple days because of API daily limits.
...
#### NYT Movie Review Data[¶](#NYT-Movie-Review-Data)
...
<pre><span></span><span class="c1"># Original Query to Pull Data from the NYT Movie Review API (one decade)</span>
...
<pre><span></span><span class="n">movielens_database</span> <span class="o">=</span> <span class="n">pd</span><span class="o">.</span><span class="n">read_csv</span><span class="p">(</span><span class="s1">'https://github.com/daveveitch/UofT/raw/master/STA2453/Project%201/Final%20Report/movies_metadata_edited.csv'</span><span class="p">)</span>
</pre>
...
#### Budget & Revenue Data (The Numbers)[¶](#Budget-&-Revenue-Data-(The-Numbers))
...
<pre><span></span><span class="n">budget_revenue</span> <span class="o">=</span> <span class="n">pd</span><span class="o">.</span><span class="n">read_csv</span><span class="p">(</span><span class="s1">'https://github.com/daveveitch/UofT/raw/master/STA2453/Project%201/Final%20Report/MovieRevenueAndBudget.csv'</span><span class="p">,</span><span class="n">encoding</span> <span class="o">=</span> <span class="s2">"ISO-8859-1"</span><span class="p">)</span>
...
<pre><span></span><span class="n">CPI</span> <span class="o">=</span> <span class="n">pd</span><span class="o">.</span><span class="n">read_csv</span><span class="p">(</span><span class="s1">'https://github.com/daveveitch/UofT/raw/master/STA2453/Project%201/Final%20Report/USCPI.csv'</span><span class="p">)</span>
...
<pre><span></span><span class="n">omdb_df</span> <span class="o">=</span> <span class="n">pd</span><span class="o">.</span><span class="n">read_csv</span><span class="p">(</span><span class="s1">'https://github.com/daveveitch/UofT/raw/master/STA2453/Project%201/Final%20Report/OscarNomineeOMDB.csv'</span><span class="p">)</span>
</pre>
...
<pre><span></span><span class="n">oscar_data</span> <span class="o">=</span> <span class="n">pd</span><span class="o">.</span><span class="n">read_csv</span><span class="p">(</span><span class="s1">'https://github.com/daveveitch/UofT/raw/master/STA2453/Project%201/Final%20Report/OscarNominees.csv'</span><span class="p">)</span>
</pre>
...
# 1\. Are film critics from the NYT able to predict box office hits?[¶](#1.-Are-film-critics-from-the-NYT-able-to-predict-box-office-hits?)
...
<pre><span></span><span class="c1"># Merge Budget and Revenue Data with NYT data</span>
...
#### Method 1 - Multiple Linear Regression - All Movies[¶](#Method-1---Multiple-Linear-Regression---All-Movies)
...
<pre><span></span><span class="n">X_cols</span> <span class="o">=</span> <span class="p">[</span><span class="s1">'infl_adj_budget'</span><span class="p">,</span><span class="s1">'critics_pick'</span><span class="p">]</span>
...
<div class="alert alert-block alert-success">**Conclusion 1 - Multiple Linear Regression:** Observing the critics_pick variable in the regression output we notice it is statistically significant and is positively associated with movies' inflation adjusted domestic gross. Based on this test it appears NYT critics are successful at choosing box office hits, and that being rated as a critics' pick is associated with a movie earning roughly \$18mm more (in 2015 dollars) at the box office than a movie that was not a critics' pick.</div>
...
#### Conclusion 1 - Limitations

It is worth noting that this simple model was only able to achieve a R-squared (the percent of variation in the data explained by the model) of 36.5%. This speaks to the fact that there are many other factors which affect movies' box office success, and the model is only able to capture a portion of this.
...
#### Method 2 - Logistic Regression - Big Budget Movies[¶](#Method-2---Logistic-Regression----Big-Budget-Movies)
...
<pre><span></span><span class="n">big_films</span> <span class="o">=</span> <span class="n">result</span><span class="p">[</span><span class="n">result</span><span class="p">[</span><span class="s1">'infl_adj_budget'</span><span class="p">]</span><span class="o">></span><span class="mi">50</span><span class="p">]</span>
...
<div class="alert alert-block alert-success">**Conclusion 2 - Logistic Regression - Big Budget Movies:** Logistic regression shows a fairly significant positive relationship between a movie being a critics' pick and it achieving financial success. According to the model, a movie that is rated a critics' pick has a 42% chance of being financially successful, versus only having a 22% chance of being financially successful if it is not rated a critics' pick. Therefore it does appear that NYT critics are able to predict which movies will be financially succesful.</div>
...
# 2\. What kind of storylines do NYT critics favour?[¶](#2.-What-kind-of-storylines-do-NYT-critics-favour?)
...
#### Method 3 - Classification Trees - Bigrams[¶](#Method-3---Classification-Trees---Bigrams)
...
<center>$ \begin{bmatrix} & dog \: runs & runs \: away & away \: fast & crew \: sails & sails \: away \\ MovieA & 1 & 1 & 1 & 0 & 0 \\ MovieB & 0 & 0 & 1 & 1 & 1 \end{bmatrix} $</center>
...
<pre><span></span><span class="kn">from</span> <span class="nn">sklearn.feature_extraction.text</span> <span class="k">import</span> <span class="n">CountVectorizer</span>
...
<pre><span></span><span class="n">movie_plot_df3</span> <span class="o">=</span> <span class="n">movielens_database</span>
...
<pre><span></span><span class="c1"># Print the classification tree</span>
...
<pre><span></span><span class="n">ftr_importances</span> <span class="o">=</span> <span class="n">model_large</span><span class="o">.</span><span class="n">feature_importances_</span>
...
# 3\. What best picture nominees tend to win the Oscar?[¶](#3.-What-best-picture-nominees-tend-to-win-the-Oscar?)
...
2.  A two sample t-test to see if movies that win the best picture Oscar have longer runtimes than those that do not win.
3.  A look at the distribution of genres for films that win the best picture Oscar versus those that do not win.

#### Method 4 - Two Sample T-Test - IMDB User Ratings[¶](#Method-4---Two-Sample-T-Test---IMDB-User-Ratings)
...
<pre><span></span><span class="n">oscar_data</span> <span class="o">=</span> <span class="n">oscar_data</span><span class="p">[</span><span class="n">oscar_data</span><span class="p">[</span><span class="s1">'category'</span><span class="p">]</span><span class="o">==</span><span class="s1">'BEST PICTURE'</span><span class="p">]</span>
...
<pre><span></span><span class="kn">from</span> <span class="nn">decimal</span> <span class="k">import</span> <span class="o">*</span>
...
<pre><span></span><span class="kn">import</span> <span class="nn">plotly.plotly</span> <span class="k">as</span> <span class="nn">py</span>
...
<div class="output_html rendered_html output_subarea output_execute_result"><iframe id="igraph" scrolling="no" style="border:none;" seamless="seamless" src="https://plot.ly/~daveveitch/8.embed" height="525px" width="100%"></iframe></div>
...
#### Conclusion 4 - Two Sample T-Test - IMDB User Ratings

We conclude that yes, the movies that the American Academy of Arts and Sciences choose to win best picture tend to have higher IMDB user ratings relative to the best picture losers. This would suggest the Academy has broadly similar preferences to the general public.
...
#### Method 5 - Two Sample T-Test - Movie Runtimes[¶](#Method-5---Two-Sample-T-Test---Movie-Runtimes)
...
<pre><span></span><span class="nb">print</span><span class="p">(</span><span class="s1">'Two Sample T-Test Results - Movie Runtimes'</span><span class="p">)</span>
<span class="nb">print</span><span class="p">(</span><span class="s1">''</span><span class="p">)</span>
...
<div class="output_html rendered_html output_subarea output_execute_result"><iframe id="igraph" scrolling="no" style="border:none;" seamless="seamless" src="https://plot.ly/~daveveitch/2.embed" height="525px" width="100%"></iframe></div>
...
#### Conclusion 5 - Two Sample T-Test Results - Movie Runtimes

Using a two sample t-test we see that the runtime of Oscar best picture winning movies are statistically significantly higher than those that do not win.
...
#### Method 6 - Distribution of Movie Genres[¶](#Method-6---Distribution-of-Movie-Genres)
...
<pre><span></span><span class="c1"># Differences in Genres</span>
